...
上面这条本地路径命令要在你自己插件的目录里执行，不是在日志包目录里。装完确认法：宿主侧能引用到已构建产物（`dist/host.js` 与 `dist/client.js`），并且读到 `hostLog.phoneNames` 里的 5 个电话名（`logBatch`、`logExport`、`logClear`、`logGetSwitch`、`logSetSwitch` 各一个），即算装好。
