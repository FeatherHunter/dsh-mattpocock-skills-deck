/**
 * tracker/preflight.js — detect / preflight / 错误 kind 分类。
 *
 * 这里是「前置检查」的通用骨架：约定错误分类与统一错误形状，供各后端 preflight 复用。
 * 第一性原理：错误分类必须把「环境问题（缺工具/缺文件）」与「资源不存在（404）」
...
export function classifyError(err) {
  ...
  if (err && ERROR_KIND[err.kind]) return err.kind
  if (err && err.error && ERROR_KIND[err.error.kind]) return err.error.kind
