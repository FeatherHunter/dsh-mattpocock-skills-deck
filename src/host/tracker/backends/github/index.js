...
export const prompts = (function () {
  ...
  return {
    ...
    healthCheck: {
      zh: '**游离的开放票**在这条后端上的落地口径：这张票**没有关闭**、**自己不是地图**（不带 wayfinder:map）、**不在任何地图的子票里** —— 这三条与列表里「独立票」的显示一致。\n\n**动手时只做两件结构动作**：\n1. 建原生子议题边，把票挂到合适的地图下：{subIssue}\n2. 为人点过头的票开一张新地图：先写最小的 Destination 草稿，正文由维护者或负责人补。\n\n**不碰**：已关闭的票、任何票的标题与正文、标签与认领状态、别的仓库；不关票、不删评论、不改源码、不发版、不提交。\n\n报告按标签与建议类型分区，但体检的范围是全仓库的开放票、不跟随面板当前的筛选。',
      en: '**How "orphaned open ticket" lands on this backend**: the ticket is **not closed**, it is **not itself a map** (no wayfinder:map label), and it is **not inside any map as a sub-ticket** — the same three facts the list uses to show a "standalone ticket".\n\n**When acting, only two structural actions**:\n1. Create the native sub-ticket edge that attaches the ticket under a suitable map: {subIssue}\n2. Open a new map for a ticket the human has nodded on: write only the minimal Destination draft and let the maintainer or owner fill in the body.\n\n**Never touch**: closed tickets, any ticket title or body, labels and claim state, other repositories; do not close tickets, delete comments, change source code, publish a release, or commit.\n\nGroup the report by label and suggestion type, but the health check always covers every open ticket in the repository and does not follow the current panel filters.',
    },
