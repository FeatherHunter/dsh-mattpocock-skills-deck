...
export const prompts = (function () {
  ...
  return {
    ...
    subIssue: {
      // #588/#600 收敛：两步调安装目录下的脚本（第 ① 步的 dsh plugin 必须带 --profile 才拿得到安装目录，第 ② 步直连脚本；--body-file 必须用绝对路径）
      zh: '先跑 `gh auth status`，没登录先按 ghAuthLogin 指引登完再继续；第 ① 步跑 `dsh plugin --profile <配置名> exec node -e "console.log(require.resolve(\'dsh-mattpocock-skills-deck/package.json\'))"` 拿插件安装目录（下面叫 <插件目录>；<配置名> 填你启动这个界面时用的那个 profile 名，Web 界面通常就是 web；`--profile` 不能省，省了 dsh 会直接报错；输出有多行时，取以 package.json 结尾的那一行）；第 ② 步跑 `node "<插件目录>/scripts/wire-subissues.mjs" --map <地图号> --children <子票号列表> --body-file <绝对路径地图正文文件>`（脚本自己取数据库编号、建原生边、校验计数；正文文件里要有 `## Destination` 一节；先确认当前目录是你的工作区（有主锚文件 docs/agents/issue-tracker.md），不要 cd 进插件目录；第 ② 步的当前目录可能是插件目录而不是你的工作区，所以 `--body-file` 必须用绝对路径）',
      en: 'Run `gh auth status` first; if not logged in, follow the ghAuthLogin guide to log in, then continue; step 1, run `dsh plugin --profile <profile> exec node -e "console.log(require.resolve(\'dsh-mattpocock-skills-deck/package.json\'))"` to resolve the plugin install dir (called <pluginDir> below; `<profile>` is the profile you booted this UI with — it is `web` for the web UI, and `--profile` cannot be omitted: dsh fails right away without it; if the output has multiple lines, take the one ending with package.json); step 2, run `node "<pluginDir>/scripts/wire-subissues.mjs" --map <map> --children <child list> --body-file <absolute-path-map-body-file>` (the script resolves the database ids, creates the native edges, and verifies the count; the body file must keep the `## Destination` section; First confirm the working directory is your workspace (it has the anchor file docs/agents/issue-tracker.md); do not cd into the plugin directory; step 2 may run with the plugin directory (not your workspace) as the working directory, so `--body-file` must be an absolute path)'
    },
    // #595：正文格式契约归后端单源（GitHub 是用 gh 的远端后端：先确认登录，再两步调安装目录下的写回脚本）；#600：第 ① 步的 dsh plugin 带 --profile（那是个必填选项，省了照抄就报错）
    bodyFormat: {
      zh: '## 正文格式（写/改 issue 正文时必须遵守）\n- [ ] 先跑 `gh auth status`，没登录先按 ghAuthLogin 指引登完再继续\n- [ ] 正文先写成文件（文件里是真实换行：每个 `## 章节` 独占一行、段落间留空行），不要把正文拼进命令行\n- [ ] 第 ① 步跑 `dsh plugin --profile <配置名> exec node -e "console.log(require.resolve(\'dsh-mattpocock-skills-deck/package.json\'))"` 拿插件安装目录（下面叫 <插件目录>；<配置名> 填你启动这个界面时用的那个 profile 名，Web 界面通常就是 web；`--profile` 不能省，省了 dsh 会直接报错；输出有多行时，取以 package.json 结尾的那一行）；第 ② 步写回：跑 `node "<插件目录>/scripts/fix-issue-body.mjs" --issue <号> --body-file <绝对路径文件>`；先确认当前目录是你的工作区（有主锚文件 docs/agents/issue-tracker.md），不要 cd 进插件目录；第 ② 步的当前目录可能是插件目录而不是你的工作区，所以 `--body-file` 必须用绝对路径；脚本回包 ok 为真才算写完（剥开头不可见字符、按阈值还原字面转义、格式只告警不改写，都由脚本负责）\n- [ ] 换行不要写成反斜杠加 n 两个字符',
      en: '## Body format (mandatory when writing/editing an issue body)\n- [ ] Run `gh auth status` first; if not logged in, follow the ghAuthLogin guide to log in, then continue\n- [ ] Write the body to a file first (real newlines in the file: each `## section` on its own line, a blank line between paragraphs); never inline the body into the command line\n- [ ] Step 1: resolve the plugin install dir by running `dsh plugin --profile <profile> exec node -e "console.log(require.resolve(\'dsh-mattpocock-skills-deck/package.json\'))"` (called <pluginDir> below; `<profile>` is the profile you booted this UI with — it is `web` for the web UI, and `--profile` cannot be omitted: dsh fails right away without it; if the output has multiple lines, take the one ending with package.json); step 2: run `node "<pluginDir>/scripts/fix-issue-body.mjs" --issue <issue> --body-file <absolute-path-file>`; confirm the working directory is your workspace (it has the anchor file docs/agents/issue-tracker.md) first, do not cd into the plugin directory; step 2 may run with the plugin directory (not your workspace) as the working directory, so `--body-file` must be an absolute path; only when the script returns ok true is the write done (the script strips the leading invisible character, restores literal escapes within the threshold, and only warns about formatting)\n- [ ] Never write a newline as the two characters backslash-n.',
    },
    errorKinds: {
