/**
 * platform/linux/index.js — Linux 适配器（通用层骨架）。
 *
 ...
 *
 * OS 专属行为（D2 getHome POSIX 优先级、resolveExecutable 别名 sh→sh 等）归 **linux 底座 map**
 * （作为 #113 子票另行规划）；本票只留最小结构 + TODO 占位。
 */
import nodePath from 'node:path'

export default function linuxAdapter(ctx) {
  void ctx
  return {
    os: 'linux',
    /** 路径数学委托 node:path（POSIX；各 OS 不得重实现）。 */
    pathImpl: nodePath.posix,
    /** 用户主目录（跨 OS 单点；OS 专属优先级细节待 linux 底座 map）。 */
    async getHome() {
      // TODO(linux 底座 map)：D2 POSIX 优先级——os.homedir() 即 $HOME，归一化为本机路径形态。
