...
//   直到只剩图标」的完整读法 —— 前半句「默认折叠」不是「起手就折叠、任何宽度都不显示」（上一版照字面
//   落成了那样：真机上那串字一个宽度都看不见，只剩一枚罗盘图标），而是「宽度紧张时它是**第一个**让步的」。
//   所以：第 0 档（最宽那一档）品牌那串字是**可见**的；宽度一不够，先收它，一次少一个字，它收完了才轮到
//   2..9（见 CAP_FOLD_POLICY[1] 旁边那段注解）。落到本文档上就是下面三条读法：
//
