...
//   的 leaf 标记处（一源两物）。
//
// 维护者这一轮的要求（原话）：「品牌字默认折叠；内容平铺在这一条里；变窄时字一个一个减少
//   （先消失非核心的），直到只剩图标」。落到本文档上就是下面三条读法：
//
//   ① 品牌那一段（priority 1）默认就是收起的 —— 阶梯的第 0 档已经按 pinned 收过，
//      它在任何宽度上都不占位置；那枚品牌图标不在本文件的台阶里（它不是「一串字」，谁也收不走它）。
 ...
 *   · steps[0] 是第 0 档（最宽那一档）—— 它已经按 pinned 收过，也就是品牌那一段已经是空的；
