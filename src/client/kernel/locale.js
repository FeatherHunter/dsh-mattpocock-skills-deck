 ...
 */
    export const L = {
      zh: {
        ...
        'tpl.name.diagnose': '诊断', 'tpl.name.fix': '修复', 'tpl.name.discuss': '讨论', 'tpl.name.research': '研究',
      ...
      en: {
        ...
        'setup.markdown.labelReqs': '',
        // #323 (2026-08-29): local Markdown label palette rule — aligned with #312/#322: Color column on existing 3-col table, names only, grey #cccccc fallback, relabel = rewrite row, recolour = table only
        'setup.markdown.paletteNote': 'Local Markdown labels use a palette: a ticket writes only label names on the `Labels:` line (e.g. `Labels: wayfinder:grilling, bug`), never colors; look up the color in the palette table inside `docs/agents/triage-labels.md`. The palette table is a four-column table that adds a `Color` column (hex value, e.g. `#8b5cf6`) on the left of the existing three columns (mattpocock role / our tracker label / meaning), one row per label; in this repo the two label-name columns are 1:1 (canonical role name = actual string); if another repo remaps, that repo table wins. Fixed values for this repo (from this repo GitHub labels): wayfinder:map=#8b5cf6, wayfinder:research=#0ea5e9, wayfinder:prototype=#f59e0b, wayfinder:grilling=#9d7cd8, wayfinder:task=#10b981, bug=#d73a4a, needs-triage=#fbca04, needs-info=#5319e7, ready-for-agent=#0e8a16, ready-for-human=#b60205, wontfix=#ffffff; add custom labels as new rows there. During setup make sure `docs/agents/triage-labels.md` already contains this palette table: if the file only has three columns or lacks a Color column, add a Color column to each row and append the rows above with their fixed colors — keep the existing columns; if the table already exists, just verify it. A name missing from the table renders grey `#cccccc`; changing a ticket labels means rewriting that ticket `Labels:` line names; change a color once, in that table.',
        ...
        'tpl.name.diagnose': 'Diagnose', 'tpl.name.fix': 'Fix', 'tpl.name.discuss': 'Discuss', 'tpl.name.research': 'Research',
