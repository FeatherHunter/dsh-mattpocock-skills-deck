 ...
 */
    export const L_PANEL = {
      zh: {
        ...
        // #230（D10 · 2026-08-28 生效）· #664：setup 提示词后端描述数据 —— 后端经 BackendModule.setupPrompt 声明键，双语单源于此；GitHub 那份 backendNote 删掉了「仓库还没就绪就先停下」那段告诫（界面已保证过），只留「本次已选后端：GitHub。」。
        ...
        'panel.closeTitle': '关闭面板', 'panel.openUnavailable': '面板没打开：两个入口现在都不可用（既没有 DSH 右侧边栏控制器，也没有 BetterSidebar）。',
      ...
      en: {
        ...
        // #230 (D10) · #664: backend-declared setup-prompt description data — keys resolved here, bilingual single source; the GitHub note lost its "repo not ready yet, stop" reminder (the UI already guarantees it).
        ...
        'panel.closeTitle': 'Close panel', 'panel.openUnavailable': 'The panel did not open: both entries are unavailable right now (no DSH right-sidebar controller and no BetterSidebar).',
