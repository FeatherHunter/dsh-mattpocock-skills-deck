 ...
 */
export     const Dot = ({ level }) => { const cx = React.useContext(DswsCtx); if (!cx) return null; const h = cx.h; return h('span', { className: 'dsws-dot', style: { background: level === 'ok' ? '#4ade80' : level === 'warn' ? '#f59e0b' : level === 'bad' ? '#f87171' : '#52525b' } }) }
