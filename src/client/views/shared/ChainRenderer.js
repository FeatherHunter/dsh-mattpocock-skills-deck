        ...
        try{
          ...
          if (!res.ok) { try{ flash(st, String((res.error && res.error.message) || tr('env.formSubmitFail')), 'warn') }catch(e){} }
            ...
            try{
              ...
              else if (typeof host !== 'undefined' && host.call) { await host.call('wf.detect', { cwd: st.cwd||'', force:true, backendId:(typeof userHintOf === 'function' ? userHintOf(st.selection) : undefined) || undefined, baseRev:(typeof baseRevOf === 'function' ? baseRevOf(st.selection) : 0) }); if(st.cwd) { try{ loadSnapshot(st,true,true) }catch(e){}; try{ loadChain(st,true) }catch(e){} } } // hint 只报「用户亲手选过的那条」：#669 第 6 件 / ADR 20260921
            }catch(e){}
          }
