 ...
 */
    ...
    function ChainSteps({ snapshot }) {
      ...
      return h('div', { className:'dsws-chain-steps', style:{ display:'flex', alignItems:'center', gap:6, padding:'6px 8px', overflowX:'auto', borderBottom:'1px solid rgba(255,255,255,.06)', marginBottom:6 } }, steps.map(function(s, idx){
        ...
        return h(Tip, { content: tr('tip.milestone', { idx: idx + 1, title: title, detail: detail ? ' — ' + detail : '' }) }, h('div', { key:s.id||idx, tabIndex:0, style:{ display:'flex', alignItems:'center', gap:6, flex:'none', border:border, borderRadius:20, padding:'2px 8px 2px 4px', background: isCurrent?'rgba(245,158,11,.12)':'rgba(255,255,255,.04)', minHeight:24 } }, [
