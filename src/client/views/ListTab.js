      ...
      const issueRow = function (x, isOpen, narrow) {
        ...
        return h('div', {
          ...
          h('div', { style: { display: 'flex', alignItems: 'flex-start', gap: 8, width: '100%' } }, [
            ...
            h('span', { className: 'dsws-tt-wrap', style: { flex: 1, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', fontWeight: isMap ? 600 : undefined, color: isOpen ? undefined : 'var(--dsw-alias-label-secondary,#a1a1aa)' }, title: x.title }, [h('span', { style: { flex: 1, minWidth: 0 } }, x.title), (x.author && x.author.login && x.author.login !== ((st.snapshot && st.snapshot.viewerLogin) || '')) ? (x.author.avatarUrl ? h('img', { src: x.author.avatarUrl, style: { width: 16, height: 16, borderRadius: '50%', border: '2px solid ' + authorColor(x.author.login), flex: 'none' }, title: '@' + x.author.login, alt: x.author.login }) : h('span', { style: { width: 16, height: 16, borderRadius: '50%', background: hexA(authorColor(x.author.login), 0.18), border: '2px solid ' + authorColor(x.author.login), display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flex: 'none' }, title: '@' + x.author.login }, [Ic({ n: 'person', size: 10 })])) : null]),
