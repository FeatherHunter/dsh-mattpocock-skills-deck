        ...
        h('div', { className: 'dsws-tt' }, [
          h('div', { className: 'dsws-tt-name' }, [
            ...
            h('span', { className: 'dsws-tt-wrap', style: { flex: 1, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' } }, [
              ...
              (t.author && t.author.login && t.author.login !== ((st.snapshot && st.snapshot.viewerLogin) || '')) ? (t.author.avatarUrl ? h('img', { src: t.author.avatarUrl, style: { width: 16, height: 16, borderRadius: '50%', border: '2px solid ' + ticketAuthorColor(t.author.login), flex: 'none' }, title: '@' + t.author.login, alt: t.author.login }) : h('span', { style: { width: 16, height: 16, borderRadius: '50%', background: hexA(ticketAuthorColor(t.author.login), 0.18), border: '2px solid ' + ticketAuthorColor(t.author.login), display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flex: 'none' }, title: '@' + t.author.login }, [Ic({ n: 'person', size: 10 })])) : null
            ]),
          ]),
