 ...
 * 「为什么、下一步怎么做」交给下面那个「后端原话」框；对「这一步没定下用哪个后端」这一档另外补一句
 ...
 * 话（词条 lc.errWaitBackend，与后端原话「稍等片刻再试」一个意思）。
 */
...
export const LabelColorDialog = (props) => {
  ...
  if (loading) {
    ...
    // 「这一步没定下用哪个后端」这一档（#631 三、）：后端原话说「先在面板里选定这个工作区用哪个后端」，
    ...
    //   话——「正在识别，等它出结果再点重试」，与后端原话「稍等片刻再试」一个意思。
