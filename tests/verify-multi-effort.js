...
const { listIssues, getIssue, createIssue, closeIssue, reopenIssue, updateIssue } = await import(pathToFileURL(resolve(ROOT, 'src/host/tracker/backends/markdown/issues.js')).href)
...
console.log('== ① 多 effort：地图各自只挂本 effort 的票 ==')
{
  ...
  const ro2 = await reopenIssue(ctx, repo, '01')
  check(ro2.ok === false && ro2.error && ro2.error.kind === 'conflict', `reopenIssue 无 effort 返回 conflict（实际 ${ro2.ok ? 'ok' : ro2.error.kind}）`)
  const up2 = await updateIssue(ctx, repo, '01', { title: '不该落盘的标题' })
  check(up2.ok === false && up2.error && up2.error.kind === 'conflict', `updateIssue 无 effort 返回 conflict（实际 ${up2.ok ? 'ok' : up2.error.kind}）`)
  check(!fs.readFileSync(betaFile, 'utf8').includes('不该落盘的评论'), 'conflict 时不写任何文件')
  check(!fs.readFileSync(betaFile, 'utf8').includes('不该落盘的标题'), 'conflict 时更新不落盘')
  // 带 effort 的重开与更新走通（写路径全覆盖：关→开→改同一张票）
  const clB = await closeIssue(ctx, { ...repo, effortId: 'beta' }, '01')
  check(clB.ok === true, 'closeIssue(beta#01) 成功')
  const roB = await reopenIssue(ctx, { ...repo, effortId: 'beta' }, '01')
  check(roB.ok === true, 'reopenIssue(beta#01) 成功')
  const upB = await updateIssue(ctx, { ...repo, effortId: 'beta' }, '01', { title: 'Beta First Renamed' })
  check(upB.ok === true, 'updateIssue(beta#01) 成功')
  check(fs.readFileSync(betaFile, 'utf8').includes('Beta First Renamed'), '更新落在 beta 文件里')
  check(fs.readFileSync(alphaFile, 'utf8') === beforeAlpha, 'alpha 文件始终未被改动')
