...
async function main() {
  ...
  const need = ['const DswsCtx = React.createContext(null)', 'function createCx(deps)', 'const cx = createCx(', 'withCx(StatusBar)', 'withCx(RunPanel)', 'withCx(SettingsPage)', 'withCx(DeckSidebarTab)', 'withCx(DeckNativeTabTitle)']
  need.forEach((k) => {
