// verify-bug-entry.js — 新增BUG入口契约（issue #4 · v2 修 #1 BUG3：7 字段挪到模板末尾 · v3 UX：宽度自适应 + 按钮 hover 反馈 · #14 契约升级（issue 称 v2→v3）：字段集精简为 4 项 + 例行指引 + EN locale 切换（v3.4：每字段「字段名：」行 + 下方「例：示例」行紧贴））
...
//  11) #14 契约升级 v3.4（2026-08-19 验收反馈重拍板）：字段集 = 期望 / 实际 / 复现步骤 / 环境信息（4 项）；指引紧贴填写位 —— 每字段「字段名：」行 + 下方一行「例：示例」（zh）/ "e.g."（en），无独立指引块；zh / en 分离（一次只出一种，跟随 DSH 语言）
const fs = require('fs')
...
// v3.4：指引紧贴填写位 —— 每字段「字段名：」行 + 下方一行「例：示例」（zh）/ "e.g." 示例行（en）；无独立指引块
...
// 例行守护：每个字段行必须紧跟「\n  例：…」示例行（zh）/「\n  e.g. …」（en）
// 注意：常量字符串里换行是字面 `\n`（两字符，JS 源码字符串转义），indexOf 用 '\\n' 匹配
const EXAMPLE_LINE_ZH = ['期望：\\n  例：应发生什么', '实际：\\n  例：看到什么', '复现步骤：\\n  例：[前置 / 场景]', '环境信息：\\n  例：OS']
const EXAMPLE_LINE_EN = ['Expected:\\n  e.g. What should happen', 'Actual:\\n  e.g. What actually happened', 'Reproduction:\\n  e.g. [Preamble / Scenario]', 'Environment:\\n  e.g. OS']
// v3.2/3.3 回归守护：独立「填写指引」块已移除（指引紧贴字段为例行）
const GUIDE_BLOCK_ZH = '\\n填写指引：'
const GUIDE_BLOCK_EN = '\\nFill-in guide:'
...
const check = function (file) {
  ...
  if (!fieldsBodyMatch) {
    ...
    // v3.4 分离守护：zh 例行不应混入英文短语（防止中英混排回潮）
    if (fieldsBody.indexOf('What should happen') >= 0 || fieldsBody.indexOf('What actually happened') >= 0) problems.push('NEW_BUG_FIELDS_BODY 混入英文 inline（v3.4 已决议 zh 只中文说明）')
    // v3.4 表单形态守护：每字段行紧跟「例：」示例行，末尾示例行收尾；无独立指引块
    const missingExZh = EXAMPLE_LINE_ZH.filter(function (g) { return fieldsBody.indexOf(g) < 0 })
    if (missingExZh.length) problems.push('NEW_BUG_FIELDS_BODY 缺例行：' + missingExZh.join(' / '))
    if (!fieldsBody.endsWith('\\n  例：OS + 浏览器 + 插件版本')) problems.push('NEW_BUG_FIELDS_BODY 末尾非「环境信息：\n  例：OS + 浏览器 + 插件版本」收尾')
    if (fieldsBody.indexOf(GUIDE_BLOCK_ZH) >= 0) problems.push('NEW_BUG_FIELDS_BODY 仍含独立「填写指引」块（v3.2/3.3 形态回潮；v3.4 指引已紧贴字段为例行）')
  }
  ...
  if (!fieldsBodyEnMatch) {
    ...
    // v3.4 分离守护：en 例行不应混入中文（跟随 DSH 语言一次只出一种）
    if (fieldsBodyEn.indexOf('应发生什么') >= 0 || fieldsBodyEn.indexOf('实际看到了什么') >= 0) problems.push('NEW_BUG_FIELDS_BODY_EN 混入中文说明（v3.4 已决议 en 只英文说明）')
    ...
    // v3.4 表单形态守护：每字段行紧跟 "e.g." 示例行，末尾示例行收尾；无独立指引块
    const missingExEn = EXAMPLE_LINE_EN.filter(function (g) { return fieldsBodyEn.indexOf(g) < 0 })
    if (missingExEn.length) problems.push('NEW_BUG_FIELDS_BODY_EN 缺例行：' + missingExEn.join(' / '))
    if (!fieldsBodyEn.endsWith('\\n  e.g. OS + browser + plugin version')) problems.push('NEW_BUG_FIELDS_BODY_EN 末尾非「Environment:\n  e.g. OS + browser + plugin version」收尾')
    if (fieldsBodyEn.indexOf(GUIDE_BLOCK_EN) >= 0) problems.push('NEW_BUG_FIELDS_BODY_EN 仍含独立 "Fill-in guide:" 块（v3.2/3.3 形态回潮）')
  }
  // 2) i18n 键
