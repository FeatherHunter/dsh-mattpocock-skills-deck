...
// ---------- A. locale 键集合 ----------
const LOCALE_SRC_FILES = ['src/client/kernel/locale-panel.js', 'src/client/kernel/locale-flow.js', 'src/client/kernel/locale-word.js', 'src/client/kernel/locale-labels.js', 'src/client/kernel/locale.js'] // #458 K5：locale.js 已拆为三片段加合并器，此处读四文件拼起来的内容断言（panel 含导航面板横幅环境引导，flow 含动作类型列表配置详情地图提示，word 含技能检查浮层命名切换进度错误模板运行描述，合并器含 L 合并逻辑）；#621 起多一份 locale-labels.js（标签配色弹窗的中英词条），同样按「每个键 zh/en 各出现一次」判定
...
function inspect(file) {
  ...
  if (rel === 'kernel/locale.js' || rel === 'kernel/locale-panel.js' || rel === 'kernel/locale-flow.js' || rel === 'kernel/locale-word.js' || rel === 'kernel/locale-labels.js' || rel === 'kernel/prompts.js') return // #458 K5：三片段与合并器均为双语定义本体，不入清单（与原 locale.js 同口径）；#621 的 locale-labels.js 同为此类
