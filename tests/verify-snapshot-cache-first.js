...
//
// 两边都要钉：既要「新鲜时不核对」，也要「过期时仍然核对」。
//   只钉前者的话，「干脆永远不核对、永远发旧数据」这种改法也能通过，那会让面板永远显示旧数据。
//
...
// ---------- 二、边界：刚好在缓存有效期内 ----------
{
  const { h, probe } = makeHost(59000)   // 59 秒，仍小于 60 秒
  const got = await h.handleSnapshot({ cwd: CWD })
  check(got === snapshot && probe.validateCalled === 0, '边界（59 秒）：仍在有效期内 → 立刻交付、不核对')
}

// ---------- 三、过期缓存：必须仍然核对（钉住另一半，防「干脆永不核对」）----------
{
  const { h, probe } = makeHost(120000)  // 2 分钟，已超过 60 秒
  const got = await h.handleSnapshot({ cwd: CWD })
  check(probe.validateCalled === 1, '过期缓存：仍然去核对了（核对次数 ' + probe.validateCalled + '）—— 缓存太旧时不许直接发')
  check(got === snapshot, '过期缓存：核对通过后照样交付（行为与改动前一致）')
}
