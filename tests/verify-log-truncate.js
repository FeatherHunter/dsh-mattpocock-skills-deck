...
// 四、渲染目录日志调用点名白名单；调用行上不许对象转文本。
{
  ...
  const allowFiles = ['SettingsPage.js', 'BackendSelector.js', 'DockSync.js', 'StatusBar.js', 'StatusLogMenu.js', 'ChecksTab.js', 'useUpdatePanel.js', 'useLabelColors.js', 'labelColorPatch.js', 'LabelColorEntry.js', 'bannerChain.js'] // #498：状态栏四键菜单的目录解析失败行（无对象转文本，见无转文本断言）；#499：StatusLogMenu 失败记账行；#587：useUpdatePanel.js 就是 SettingsPage.js 里那块更新逻辑（原文件已在名单内，拆出后是同一批行，不是新增记点）；#621：useLabelColors.js 是标签配色弹窗的状态机，两条宿主电话（wf.listLabels / wf.setLabelColors）就在这个文件里，调用点相邻的那行必须记（verify-log-coverage 的调用点可追踪断言要求），行上没有对象转文本；#635：labelColorPatch.js 是保存成功后写进面板快照的那笔颜色记录的纯函数模块（住 views 目录只是因为标签配色这几个叶子都在这里，它自己不渲染任何东西），新增的按需事件 #62 labelColors.panelPatch 就记在这里，三次调用只带工作区键散列、枚举与条数，没有对象转文本；#637：LabelColorEntry.js 是面板头部那颗入口按钮，挂载时为给图标取真实颜色调一次既有的 wf.listLabels（与 useLabelColors.js 里那次同一条电话，同一套记账：成功记 info host.call、失败记 warn host.call.fail，不新增日志事件），调用点相邻的那行必须记（verify-log-coverage 的调用点可追踪断言要求），行上没有对象转文本；#663：bannerChain.js 是状态栏那条横幅的纯逻辑叶子（住 statusbar 目录，自己不渲染任何东西；与 labelColorPatch.js 同一类）—— 新立的常驻事件 #65 guide.inject 就记在这里（用户点一次落一行，只带两个短枚举：哪一步、给出去的是哪一类），行上没有对象转文本；#685：「体检」按钮那条常驻事件 #68 healthCheck.inject 同理落在内核文件 src/client/kernel/health-check.js（内核目录不在本白名单的管辖范围里，所以既没把 ListTab.js 加进名单，也没在这张表动一个字），行上只有两个短枚举，没有对象转文本
  const seen = {}
