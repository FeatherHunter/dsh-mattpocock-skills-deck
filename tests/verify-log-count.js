// verify-log-count.js —— #494 第三件事：日志门禁之计数（#489 附录第 4 节断言六，#499 自监控 5 条并入，#548 安装执行 1 条并入）。
// 用法：在插件根目录执行 node tests/verify-log-count.js，可独立运行。
// 断言文字：常驻 30 条、按需 20 条、自监控 5 条、总数 55（#22、#34 已退役）；
// 增删事件必须同步更新附录对照表，否则红。
...
console.log('日志计数门禁（#494/#498/#499/#548：常驻 30、按需 20、自监控 5、总数 55，与附录修订版字面一致）')
...
if (appendix) {
  check(appendix.includes('常驻 30 条、按需 20 条、自监控 5 条、总数 55 条'), '附录 counts 字面为常驻 30 条、按需 20 条、自监控 5 条、总数 55 条')
...
// 三、总数：已知事件恰为 55 个（常驻 30 加按需 20 加自监控 5），退役的不计入。
{
  ...
  check(RESIDENT.length === 30 && ONDEMAND.length === 20 && SELFMON.length === 5 && known.length === 55, '清单总数 55（常驻 30、按需 20、自监控 5）')
  check(missing.length === 0, '55 个事件全部落点无缺口' + (missing.length ? ' —— 缺口：' + missing.join('、') : '（命中 ' + hitKnown.length + ' 个）'))
}
