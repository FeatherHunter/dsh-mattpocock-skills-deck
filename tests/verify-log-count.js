...
// 断言文字：常驻 30 条、按需 22 条、自监控 5 条、总数 57（#22、#34 已退役）；
// 增删事件必须同步更新附录对照表，否则红。
...
console.log('日志计数门禁（#494/#498/#499/#548/#606：常驻 30、按需 22、自监控 5、总数 57，与附录修订版字面一致）')

// 附录对照表里的现行清单（1.3 落定后常驻 27 条，#498 增补 #51 与 #55 成 29 条，#548 增补 #57 成 30 条；另有 2 条已退役只作追溯）。
const RESIDENT = ['snapshot.request', 'snapshot.cache.miss', 'repo.resolve.tier', 'gh.exec', 'gh.timeout', 'gh.resolve.fail', 'graphql.fallback', 'issues.fallback', 'snapshot.built', 'panelSync.dirty', 'registry.select', 'detection.detect', 'skill.probe', 'skill.pending.cap', 'host.call', 'host.call.fail', 'snapshot.hydrate', 'backend.switch', 'naming.guard', 'naming.lock', 'settings.save', 'panel.open', 'statusbar.fallback', 'dock.rehydrate', 'storage.fail', 'chain.derive.error', 'fallback.chain', 'client.snapshot.miss', 'host.start', 'update.install.exec']
// 按需 22 条编号（#498 增补 #52、#53、#54、#56，#606 增补 #58、#59）：含 #45，不含已退役。
const ONDEMAND = ['snapshot.cache.hit', 'probe.eval', 'panelSync.eval', 'registry.stub', 'workspaceStore.hit', 'chain.cache.hit', 'chain.predicate', 'workspaceKey.canonical', 'platform.resolve', 'naming.sweep', 'snapshot.fanout', 'dedup.hit', 'statusbar.hydrate', 'error.normalize', 'timer.schedule', 'privacy.scrub', 'chain.cache.miss', 'workspaceStore.miss', 'client.snapshot.hit', 'detail.cache.hit', 'exec.run', 'panel.render']
...
if (appendix) {
  check(appendix.includes('常驻 30 条、按需 22 条、自监控 5 条、总数 57 条'), '附录 counts 字面为常驻 30 条、按需 22 条、自监控 5 条、总数 57 条')
  check(appendix.includes('1、3、4、5、6、7、8、9、10、13、14、16、20、21、26、27、28、31、32、33、35、36、38、39、40、41、42、51、55、57'), '附录常驻编号清单 30 个（不含已退役的 22、34，不含 45；#498 增补 51、55，#548 增补 57）')
  check(appendix.includes('2、11、12、15、17、18、19、23、24、25、29、30、37、43、44、45、52、53、54、56、58、59'), '附录按需编号清单 22 个（含 45；#498 增补 52、53、54、56；#606 增补 58、59）')
...
// 三、总数：已知事件恰为 57 个（常驻 30 加按需 22 加自监控 5），退役的不计入。
{
  ...
  check(RESIDENT.length === 30 && ONDEMAND.length === 22 && SELFMON.length === 5 && known.length === 57, '清单总数 57（常驻 30、按需 22、自监控 5）')
  check(missing.length === 0, '57 个事件全部落点无缺口' + (missing.length ? ' —— 缺口：' + missing.join('、') : '（命中 ' + hitKnown.length + ' 个）'))
}
