// verify-log-count.js —— #494 第三件事：日志门禁之计数（#489 附录第 4 节断言六，#499 自监控 5 条并入，#548 安装执行 1 条并入，#606 按需 2 条并入）。
// 用法：在插件根目录执行 node tests/verify-log-count.js，可独立运行。
// 断言文字：常驻 31 条、按需 23 条、自监控 5 条、总数 59（#22、#34 已退役；#618 增补常驻 #60 与按需 #61）；
// 增删事件必须同步更新附录对照表，否则红。
...
console.log('日志计数门禁（#494/#498/#499/#548/#606/#618：常驻 31、按需 23、自监控 5、总数 59，与附录修订版字面一致）')
...
// 按需 23 条编号（#498 增补 #52、#53、#54、#56，#606 增补 #58、#59，#618 增补 #61）：含 #45，不含已退役。
const ONDEMAND = ['snapshot.cache.hit', 'probe.eval', 'panelSync.eval', 'registry.stub', 'workspaceStore.hit', 'chain.cache.hit', 'chain.predicate', 'workspaceKey.canonical', 'platform.resolve', 'naming.sweep', 'snapshot.fanout', 'dedup.hit', 'statusbar.hydrate', 'error.normalize', 'timer.schedule', 'privacy.scrub', 'chain.cache.miss', 'workspaceStore.miss', 'client.snapshot.hit', 'detail.cache.hit', 'exec.run', 'panel.render', 'labelColors.read']
...
if (appendix) {
  check(appendix.includes('常驻 31 条、按需 23 条、自监控 5 条、总数 59 条'), '附录 counts 字面为常驻 31 条、按需 23 条、自监控 5 条、总数 59 条')
  ...
  check(appendix.includes('2、11、12、15、17、18、19、23、24、25、29、30、37、43、44、45、52、53、54、56、58、59、61'), '附录按需编号清单 23 个（含 45；#498 增补 52、53、54、56；#606 增补 58、59；#618 增补 61）')
...
// 三、总数：已知事件恰为 59 个（常驻 31 加按需 23 加自监控 5），退役的不计入。
{
  ...
  check(RESIDENT.length === 31 && ONDEMAND.length === 23 && SELFMON.length === 5 && known.length === 59, '清单总数 59（常驻 31、按需 23、自监控 5）')
  check(missing.length === 0, '59 个事件全部落点无缺口' + (missing.length ? ' —— 缺口：' + missing.join('、') : '（命中 ' + hitKnown.length + ' 个）'))
}
