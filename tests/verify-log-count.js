// verify-log-count.js —— #494 第三件事：日志门禁之计数（#489 附录第 4 节断言六，#499 自监控 5 条并入，#548 安装执行 1 条并入，#606 按需 2 条并入，#635 按需 1 条并入，#652 按需 1 条并入，#655 按需 1 条并入，#663 常驻 1 条并入）。
// 用法：在插件根目录执行 node tests/verify-log-count.js，可独立运行。
// 断言文字：常驻 32 条、按需 26 条、自监控 5 条、总数 63（#22、#34 已退役；#618 增补常驻 #60 与按需 #61，#635 增补按需 #62，#652 增补按需 #63，#655 增补按需 #64，#663 增补常驻 #65）；
// 增删事件必须同步更新附录对照表，否则红。
...
console.log('日志计数门禁（#494/#498/#499/#548/#606/#618/#635/#652/#655/#663：常驻 32、按需 26、自监控 5、总数 63，与附录修订版字面一致）')

// 附录对照表里的现行清单（1.3 落定后常驻 27 条，#498 增补 #51 与 #55 成 29 条，#548 增补 #57 成 30 条，#618 增补 #60 成 31 条，
// #663 增补 #65 成 32 条；另有 2 条已退役只作追溯）。
...
if (appendix) {
  check(appendix.includes('常驻 32 条、按需 26 条、自监控 5 条、总数 63 条'), '附录 counts 字面为常驻 32 条、按需 26 条、自监控 5 条、总数 63 条')
  check(appendix.includes('1、3、4、5、6、7、8、9、10、13、14、16、20、21、26、27、28、31、32、33、35、36、38、39、40、41、42、51、55、57、60、65'), '附录常驻编号清单 32 个（不含已退役的 22、34，不含 45；#498 增补 51、55，#548 增补 57，#618 增补 60，#663 增补 65）')
...
// 三、总数：已知事件恰为 63 个（常驻 32 加按需 26 加自监控 5），退役的不计入。
{
  ...
  check(RESIDENT.length === 32 && ONDEMAND.length === 26 && SELFMON.length === 5 && known.length === 63, '清单总数 63（常驻 32、按需 26、自监控 5）')
  check(missing.length === 0, '63 个事件全部落点无缺口' + (missing.length ? ' —— 缺口：' + missing.join('、') : '（命中 ' + hitKnown.length + ' 个）'))
}
