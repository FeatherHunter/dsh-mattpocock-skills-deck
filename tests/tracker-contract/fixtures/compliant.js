 ...
 *
 * 定版形状（#127）：单 `key`(string)、无 `number`/`subIssues`；`parentKey` 核心字段；
 * 能力字段可 MISSING（本桩实现 labels/assignees/comments/blockedBy/blocking/reason → EMPTY；
...
const deckIssues = {
  ...
  claimed: {
    ...
    createdAt: '', updatedAt: '', closedAt: null, parentKey: null,
    labels: [], assignees: [{ login: 'a' }], comments: [], blockedBy: [], blocking: [], reason: '',
  },
  blockedOpen: {
    key: '13', type: 'issue', title: 'blocked', state: 'open', body: '', url: '',
    createdAt: '', updatedAt: '', closedAt: null, parentKey: null,
    labels: [], assignees: [], comments: [], blockedBy: [{ key: '9', title: 'x', state: 'open' }], blocking: [], reason: '',
  },
  indeterminateMissingAssignees: {
    key: '14', type: 'issue', title: 'indeterminate', state: 'open', body: '', url: '',
    createdAt: '', updatedAt: '', closedAt: null, parentKey: null,
    labels: [], comments: [], blockedBy: [], blocking: [], reason: '',
  ...
  notFoundDepSafeBlocked: {
    ...
    blockedBy: [{ key: 'ghost-1', title: 'missing', state: 'closed' }], blocking: [], reason: '',
  },
}
