 ...
 *  - select 三级联三态（explicit > matches > fallback；matches 超时→pending 排除决策集；
 *    平局=注册序 + multiHit 暴露；无 explicit/match/pending 才 fallback→null 且 ref 省略）。
 *  - on(unsubscribe) / Disposable / 监听抛错隔离 / describe / MIGRATE_KEY。
 */
...
export async function run() {
  ...
  await assert('pending：matches 超时 → 排除决策集 + surface pending', (async () => {
    const r = createRegistry({}, { matchesTimeout: 15 })
    r.register(mkModule('slow', { matches: () => new Promise(() => {}) }))
    const sel = await r.select(handle, { cwd: handle.cwd })
    return sel.backendId === null && Array.isArray(sel.pending) && sel.pending.includes('slow')
  })(), 'timeout must surface pending, not silently Other')
  await assert('pending 被排除：有命中时选命中但保留 pending', (async () => {
    const r = createRegistry({}, { matchesTimeout: 15 })
    r.register(mkModule('slow', { matches: () => new Promise(() => {}) }))
    r.register(mkModule('fast', { matches: async () => true }))
    const sel = await r.select(handle, { cwd: handle.cwd })
    return sel.backendId === 'fast' && Array.isArray(sel.pending) && sel.pending.includes('slow')
