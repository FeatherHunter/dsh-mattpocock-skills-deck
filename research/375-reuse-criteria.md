> **归档说明（2026-09-16 生效）**：本文件原在分支 `research/reuse-criteria` 的 `_research/reuse-criteria.md`，归档时该分支顶端提交是 `f5a8ab40be`。2026-09-16 随分支清理并进本仓库主线；正文只有第 9 行与第 66 行两处自述旧位置的路径改成了新位置，其余一字未动。
> 文中提到的分支、路径、行号与实测数字，都是归档当时的状态快照。按 CONTEXT.md 的「版本与效力」规则：本文件与标注日期更新的内容冲突时，以更新者为准，后续如有改动同样以未来版本为准，今天的内容不是永久不可变。
> 原路径落在 .gitignore 覆盖的临时目录里，归档时改放到本仓库的正式路径，正文没有改动。本文件是一次性研究结论，并进主线后不再随主线维护；需要改动时按 CONTEXT.md 的规则以更新日期的版本为准。
...
> 存放：`research/375-reuse-criteria.md` · 归档前在分支 `research/reuse-criteria`
...
| 阈值 | 含义 | 动作 | 证据标尺 |
|------|------|------|----------|
| **2 套重复即标记** | 同类逻辑出现 2 处 | 代码旁加 `// TODO reuse:<key>` 并记入 `research/374-reuse-inventory.md` 增量 | 当前即此档：鼠标跟随 vs 锚点跟随 2 套半（R1 §3 证据1） |
