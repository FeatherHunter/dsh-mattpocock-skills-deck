> **归档说明（2026-09-16 生效）**：本文件原在分支 `research/session-naming-206` 的 `.scratch/research-session-naming-204.md`，归档时该分支顶端提交是 `875338ff02`。2026-09-16 随分支清理并进本仓库主线；正文只有第 8 行一处自述旧位置的路径改成了新位置，其余一字未动。
...
> 产出路径：`research/206-session-naming.md` · 归档前在分支：`research/session-naming-206`
