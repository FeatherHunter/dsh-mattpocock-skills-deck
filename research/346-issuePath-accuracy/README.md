> **归档说明（2026-09-16 生效）**：本文件原在分支 `research/346-issuePath-accuracy` 的 `research/346-issuePath-accuracy/README.md`，归档时该分支顶端提交是 `73c478fba9`。2026-09-16 随分支清理并进本仓库主线，正文一字未动，只在本行之上加了这段说明。
> 文中提到的分支、路径、行号与实测数字，都是归档当时的状态快照。按 CONTEXT.md 的「版本与效力」规则：本文件与标注日期更新的内容冲突时，以更新者为准，后续如有改动同样以未来版本为准，今天的内容不是永久不可变。
> 本文件是一次性研究结论，并进主线后不再随主线维护；需要改动时按 CONTEXT.md 的规则以更新日期的版本为准。
