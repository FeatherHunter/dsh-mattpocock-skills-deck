...
| `kernel/store-derived.js`（标记名 `storeDerived`） | 派生统计（`compute`、`frontierAll`、`openIssuesOf`、`isOccupied`、`occCount`、`frontierCount`、`applyStandaloneBlocks`、`hasLabelOf`、`isTriageLike`、`bugCount`、`triageCount`、`buildColorOf`、`isLightHex`、`actionColorOf`、`rowActionText`、`mkRowAction`、`timeStampStr`） | issueUrlFor（link，调用时）、renderTemplate/startText（config/router，调用时） | 会话级状态之派生统计与行级动作（#455 由 `store.js` 第 600–741 行拆出，150 行；#544 增独立票阻塞边合并 `applyStandaloneBlocks`，只读快照 issues 自带边，零请求） |
