...
| `kernel/locale.js`（标记名 `locale`，合并器） | `L`（`Object.assign` 合并四片段 zh/en，key 一个不改只搬家） | `L_PANEL`/`L_FLOW`/`L_WORD`/`L_LABELS`（定义时引用，标记位顺序保证先片段后合并） | tr 绑定（`localeSvc.bind('dsws')`）由 index.js 装配；字典为唯一真源，verify-t3-locale 契约；合并器 13 行 |
