...
- **收起之后看得见回来的路**：横幅最右那颗叉会把整个功能区收起来，收起态原来只剩一句 10px 灰字（没底色、没描边），看着像注释而不像按钮；用户按下去之后整条状态栏连同胶囊一起消失、又找不到回来的入口，报成了「整个胶囊状态栏不见了」。现在收起态那颗按钮照胶囊自己的样式画（同底色、同描边、同圆角、字号 12、写全面板名），那颗叉也与左边的主按钮拉开 12 像素（#669，提交 `51372bc`）。真机实测：点主按钮胶囊仍在（877×31）；点收起后原地出现 190×25 的「展开MattSkillsDeck」；点它胶囊与横幅一起回来、收起标记被清空。
- **面板这几处**：面板头部去掉品牌图标与文字（#667）；面板只剩 DSH 原生右侧边栏一条落点，页内浮窗那条老路退役（#646）；面板第一帧不再是透亮的空块（#670）；「当前目录属于工作区 X」那枚标志修好并改红（#666）。
- **对应提交**：`2a37464`（#667 面板头）、`6d074ed`（绑完立刻重取链 + 弹窗搬到状态舱）、`311636e`（面板只留原生右栏 + 第一帧底色）、`03573dd`（#666 那枚标志）、`51372bc`（#669 收起态看得见回来的路）、`7208564`（#669 晚到的旧结果不许盖回），以及本版发布提交。
- **验证**：`node tests/verify-release-contract.js --version v1.7.23` 全绿；`npm run verify` 整条链全绿（本版新增三道门：收起态「看得见、回得来」的真浏览器几何门 27 项、点确认后立刻重取链的行为门 12 项、弹窗挂到状态舱的全屏遮罩门 11 项）；`npm run test:smoke` 全绿；`npm pack --dry-run` 只含白名单文件。真机逐条验收由维护者在本版发布前按清单走（场景、步骤与通过标准见 #669 的评论）。
- **影响**：装到本版后，新工作区第一次打开会按顺序出提示，不会提前跳黄条；建仓弹窗不再依赖右侧面板；点过收起也随时能展开回来。面板其余行为与 `1.7.22` 一致。
