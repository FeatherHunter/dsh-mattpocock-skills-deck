...
- **改了什么（按能看到的行为）**：
  - 每一次出站（跑 gh、起子进程、探测版本）都先过唯一出口闸并记账；绕开闸的存量口子从 3 处降到 **0 处**，门禁里那条上限也跟着归零——以后再有新口子会当场变红。
  - 额度的数字只有一处真源（`refresh-core/src/budget.ts`），闸与账本按「这笔出站是谁在花」分档；AI 工具的花费有单次与每小时的硬顶，超顶时**如实拒绝并把原话给出来**，不是静默失败。
  - 变化只重查真正变了的那几张票（行级增量），整池重建从「每次都做」变成「必要时才做」；水印只在变化**真的并进列表之后**才前进，被推迟或丢弃时一个字节不动。
  - 只有真的在被看的工作区才会去刷（视野模型）：后台不再空转，同一个工作区根不会被重复探测，90 秒没有信号就收摊。
  - 工具把票改了之后插件立刻知道（订阅写事件，工作区白名单是第一道门），并把这件事记进下面那条「每个会话在处理哪些票」。
  - 界面上新加一条「每个会话在处理哪些票」，数据只读宿主写下的真实读数：**读不到就如实说读不到**；宿主说取到了而确实没有，整块不画。
  - 新增七个后端无关的票务工具（看票 / 看地图 / 建票 / 建地图骨架 / 补边 / 改票 / 取上下文），每条边**如实报它落在后端哪里**（GitHub 是原生层级、GitLab 只是平级链接、本地 Markdown 做不到）。
  - 修 GitLab 建票：一次调用只发一次写。此前是先对集合发 PUT、拿不到票面再回落发 POST，而真 GitLab 对集合的 PUT 必 404，回落那段**永远走不到**——也就是说这条路此前其实是坏的；同一个幂等键第二次调用一个字节都不写。
  - 提示词与随包说明里不再写死 GitHub：工具能做的事一律指向工具，只有登录、建仓库、装命令行工具留给各后端自己说。
- **为什么这么做**：最初的问题是刷新太频繁，导致 gh 命令失败、甚至撞上配额上限。单纯把节拍调慢只能把问题往后推，所以改成结构性的做法——每次出站都要先问闸、都要记账，预算与优先顺序写在一处，界面把「什么时候取的、失败是哪一种」如实显示出来。
- **对应提交**：本版的主体提交（`refresh-core/` 的纯逻辑与它的产物、宿主侧装配 `src/host/refresh/wiring.js`、唯一出口闸与账本、行级增量、视野模型、写事件订阅、会话↔票处理链与它的展示面、七个 deck 工具、GitLab 单次写、提示词去硬编码、日志登记收口），以及本版发布提交。
- **验证**：本批新增与改动的门禁逐条单独跑过、全部 EXIT=0（`verify-gh-gateway`、`verify-policy`、`verify-gate-accounting`、`verify-budget-worstcase`、`verify-event-budget`、`verify-delta-patch`、`verify-delta-wiring`、`verify-attention-model`、`verify-deck-tools` 与三后端矩阵、`verify-deck-tools-host-wiring`、`verify-gitlab-single-write`、`verify-visible-truth`、`verify-chain-view`、`verify-723-single-refresh-instance`、`verify-723-gate-wiring`、日志家族全绿）；`node tests/verify-release-contract.js --version v1.7.28` 全绿。**如实说明**：整条 `npm run verify` 链**不是全绿**——仍有开工前就存在的红灯（`verify-no-same-layer-import` 的两条同层边，来自 2026-09-22 的提交）与 6 条与本批无关的存量红；它们逐条记在仓库的门禁清单里，本版没有把它们算作通过。
- **影响**：装到本版后，刷新的花费、时刻与失败形态都能被查到；后台不再空转、同一个工作区不会被重复探测；界面上的新鲜度与失败是宿主写下的真实值。**没有做完的部分如实写在这里**：把七个工具挂上 DSH 的模型工具面（也就是 AI 在会话里真的调得到它们）、以及「列表行不再搬正文」的薄片段优化都还没做，另有宿主入口改名的收尾；这三件连同终局真机验收，都还有跟踪票在仓库的 GitHub 上。

## 2026-09-21 · v1.7.27 发布：布局卡上的选项可以改了——记住的那一份不再把你刚点的那一下按回去

- **目的**：修 1.7.25 当天冒出来的那处「点不动」：维护者在真机上点黄条 → 卡出来了，但卡上的 single / multi **另一个选项点不动**（点了立刻被按回去）。
- **原因**：1.7.25 让卡片预选去读「这个工作区记住的那一份」，而它排在**用户刚点的那一下前面** —— 点另一个选项 → 状态确实变了 → 重画时又被记住的那份覆盖，看上去就是点不动（连卡上确认也会按记住的旧答案写）。这是那次改动引入的顺序错，不是卡片本身的问题。
- **修法**：卡片这一组单选的取值顺序改成「**卡片开着时以卡上刚点的那一下为准** → 会话里答过的 → 这个工作区记住的 → 卡上上次碰过的 → 缺省」；卡上「取消」与「确认」都把那份草稿清掉，免得下次打开时它还压着预选。
- **对应提交**：本版修复提交（`StatusBackend.js` 的取值顺序 + 门禁三条断言与一条反证），以及本版发布提交。
- **验证**：`tests/verify-665-fresh-workspace-chain.js` 新增三条行为断言 + 一条反证 —— 修之前这三条是红的（门禁先红、再修代码；反证把「卡片开着时以卡上刚点的那一下为准」那段从真源里拆掉，前三条必须变红）；`npm run verify` 整条链全绿；`npm run test:smoke` 全绿；`node tests/verify-release-contract.js --version v1.7.27` 全绿。
- **影响**：装到本版后，点黄条弹卡 → 预选着上次那一项 → 随时可以改成另一项，确认后注入的初始化全文跟着换成新选的结论；「每次都问」「按工作区记住」与 `1.7.25` 一致，别的行为不变。

## 2026-09-21 · v1.7.26 发布：声明宿主要求（市场卡片那颗「未声明宿主要求」不再出现）＋市场介绍换新、预构建包链接接上最新版

- **目的**：市场卡片底部那颗「未声明宿主要求」提示，原因是发布包清单里没有声明这个插件能在哪些 DSH 版本上跑；同一张卡片的介绍文字，维护者给了一份新的原文要换上。本版只动对外展示的三件事和一个清单字段，插件行为一个字节都不改。
- **宿主要求在哪声明、市场怎么读**：在 `package/package.json` 增加 `"engines": { "dsh": ">=0.1.5-rc.1" }`。市场（dsh-market 的发现期兼容判定）抓的是 npm 上这一版 `latest` 清单，只读两类声明——顶层 `engines.dsh`（或包自己的 `dsh.engines.dsh`）与 `peerDependencies` 里 `@deepseek-ai/dsh*` 的条目；两类都没有，才显示「未声明宿主要求」。声明之后同一位置改成显示 `DSH >=0.1.5-rc.1`，市场也能按 DSH 版本筛选插件。清单事实在本地有 24 小时缓存，所以卡片最迟在一天内变过来。
- **下限为什么取 `0.1.5-rc.1`**：与 README 写明的匹配内核一致——DSH 官方源当前的 `latest` 就是这一版，DSH Desktop 2.0.10 自带的 `0.1.5-rc.2` 是同一份代码只差版本号，同样落在区间里。
- **介绍文字换新（三处同源）**：包介绍、GitHub 仓库 About、awesome 市场条目换成维护者给的原文，逐字照录：「安装即自带mattpocock/skills v1.2.3的25个工程与效率技能，无需手动装技能。300亿token打造。本插件在原始技能之上提供10倍的开发效率。主力支持GitHub issue；Markdown为预览版；GitLab暂不支持。」
- **门禁判据同步放宽一处**：`tests/verify-release-contract.js` 原先要求介绍文字里必须出现 `DSH`/`DeepSeek` 字样；维护者指出市场条目全在 DSH 语境里、介绍不必再强调一次，于是这条改成「介绍里出现，或包自己带 dsh 身份（包名或 keywords，keywords 那条本来就有独立检查）」即为通过，判据意图（human-first 可检索）不变。
- **市场条目的预构建包链接接上最新版**：awesome 条目里那条 `tarball` 从钉死的 `v1.7.14` 换成 `https://github.com/FeatherHunter/dsh-mattpocock-skills-deck/releases/latest/download/dsh-mattpocock-skills-deck.tgz`；每次发布 Release 后 `.github/workflows/release-tarball.yml` 都会把这一版的原样产物以这个固定文件名附上去，所以这条链接以后不会再钉在旧版本上。
- **对应提交**：本版发布提交。
- **验证**：`node tests/verify-release-contract.js --version v1.7.26` 全绿；`npm run verify` 整条链全绿；`npm run test:smoke` 全绿；`npm pack --dry-run` 只含白名单文件。
- **影响**：装到本版后插件行为与 `1.7.25` 完全一致。市场卡片最迟一天内把宿主要求显示为 `DSH >=0.1.5-rc.1`；介绍换成维护者给的原文；从市场点「预构建包安装」装到的是最新版，不再是 `1.7.14`。

## 2026-09-21 · v1.7.25 发布：布局那一问点一次问一次、答案按工作区记住——删掉注入的文字再点黄条，卡还在

- **目的**：修一处真机观察到的别扭：点黄色「初始化」横幅 → 弹出「各部分共用一套用语吗」那张小卡 → 选完注入了初始化全文 → 把注入的文字删掉，再点黄条，卡片不出现了（直接又注入一段全文）。原因是 #655 定的是「布局只问一次、只活在本次会话、答过就直接注入」——答过之后界面上再没有地方能看见或改这个答案，只能刷新面板（这条出路没人知道）。
- **裁决（维护者 2026-09-21 拍板 A + 记住）**：① 黄条那颗「初始化」按钮**每次都先弹那张卡**，卡上预选着上次那一项，点确认之后才注入全文 —— 看得见、随时能改、点一次问一次；② 这个答案**按工作区记住**，同一个工作区下次打开直接沿用，不再因为换了个会话就重新问、也不会两个会话答成两个样。
- **别的入口不跟着变**：检查页红牌那颗「执行初始化」按钮、面板切换后端之后那条注入路，答过（或记住过）就直接注入 —— 它们是「要一段文字」的动作，不是「问布局」的地方，不给它们多加一次确认。
- **已知限制（如实写进决策记录）**：黄条那颗按钮每次多一次确认（这是这次裁决明码标价的代价）；答案存在这台机器的浏览器里，换机器或清缓存会重新问；用户在仓库里手改 `docs/agents/domain.md` 不会改变插件记住的答案 —— 要改就在卡上改一次。
- **对应提交**：本版修复提交（`prompts.js` 的开卡条件与 `readSetupLayout` 的持久回退、`StatusBackend.js` 的入口与落盘、`store-prefs.js` 的两张函数、四道门禁的同步更新），以及本版发布提交。
- **验证**：`node tests/verify-release-contract.js --version v1.7.25` 全绿；`npm run verify` 整条链全绿（两处门禁按新流程加重了断言：`tests/verify-655-setup-layout.js` 178 项含「每次都问」「按工作区记住」两组，`tests/verify-665-fresh-workspace-chain.js` 91 项把黄条那条路量成「点一下先弹卡、卡上确认才注入」两步）；`npm run test:smoke` 全绿；`npm pack --dry-run` 只含白名单文件。
- **影响**：装到本版后，点黄条总会先看到那张卡（预选着上次的答案），确认后注入的初始化全文里写着同一句布局结论；同一个工作区的新会话不再重问；切换后端与检查页那颗按钮的行为与 `1.7.24` 一致。

## 2026-09-21 · v1.7.24 发布：切换后端之后「谁说了算、该注入什么」一次定清楚——刚切的后端不再被顶回旧后端

- **目的**：把「切换后端」这条路上的一笔糊涂账算清：用户在面板上选的后端到底算不算数（原来不算 —— 宿主取数时先问仓库里那份 `docs/agents/issue-tracker.md`，用户刚点的那一下根本轮不到），以及切完之后该往会话里注入什么（原来可能什么都不给，也可能把初始化全文重发一遍）。真机现场是：在已经初始化过的仓库里从 GitHub 切到本地 Markdown，提示条说切了、弹窗也关了，顶部的后端标却仍是 GitHub。
- **判定顺序**：用户的手动选择 > 锚文件 > 自动识别（锚文件仍然压过自动识别）。只认用户亲手点确认的那一条（带一个标记，随选择一起持久化）；快照/链算出来的派生值不带标记、照旧排在锚后面 —— 否则锚文件写过的那个结论会被缓存下来冒充用户意图，之后谁改锚文件都不再生效。
- **那次「切完仓库标回退」的另一半原因**：客户端读快照的第一步会把内存里那份**切换前**的旧快照合并回会话，「用户刚点的那一下」在把选择上报给宿主之前就被抹掉了。这一版水合不再合并它；宿主随后的真回包照旧能纠正用户的选择。
- **切完之后按场景给**：已经初始化 → 注入一段「把仓库里记录后端的那几处（`docs/agents/issue-tracker.md`、`docs/agents/domain.md`、`AGENTS.md` 的 `## Agent skills` 块）对齐到新后端」的指令，并要求 AI 回读自证、不许重跑初始化；还没初始化 → 与黄色横幅完全同一条路（先问域文档布局，答完注入初始化全文）；仓库还没就绪、或链里暂时拿不到「初始化过没有」这一步 → 一个字都不注入，只把下一步指向状态栏。
- **按不动的按钮不再假装能按**：「清除后端选择」删掉；「迁移」「清空」置灰，卡片正文写明今天没有实现；GitLab 在三个选后端的窗（切换弹窗、面板门控窗、状态栏门控窗）里都置灰，确认那一步再挡一道；门控窗的提示条改成「接下来按提示完成初始化」—— 那个窗只在还没有后端时开，从来没有旧数据。
- **对应提交**：`933779f`（本次修复：判定顺序、水合守卫、按场景注入、三窗置灰、配套门禁与决策记录），以及本版发布提交。
- **验证**：`node tests/verify-release-contract.js --version v1.7.24` 全绿；`npm run verify` 整条链全绿（本版新增一道门禁 `tests/verify-669-choice-precedence.js`，55 项，每组都配「把实现做坏就必须变红」的反证）；`npm run test:smoke` 全绿；`npm pack --dry-run` 只含白名单文件。真机逐条验收由维护者按 `docs/reviews/669-acceptance-checklist.html` 走（六个场景 + 四条已知限制）。
- **影响**：装到本版后，切换后端立刻在顶部后端标上看得见、不会再自己弹回旧后端；已初始化的工作区切完会收到一段「把记录后端的地方对齐」的指令，不会重跑初始化；未初始化的工作区与黄条走同一条路；「迁移」「清空」「GitLab」三处都按不动了。已知限制见 `docs/adr/20260921-tracker-choice-precedence.md` 第 4 节：切换后没让 AI 跑那段对齐指令的话，仓库里那份文件仍是旧后端；用户手改锚文件不会让面板跟着变（要在面板上再切一次，或清掉这台浏览器的缓存）。

## 2026-09-20 · v1.7.23 发布：全新工作区第一次打开这条引导链走通——黄条等仓库就绪、建仓窗搬到状态舱、收起之后看得见回来的路

- **目的**：把「全新工作区第一次打开」这条路按顺序走通：先问后端 → 缺 gh 时给安装原话 → 缺远端仓库时开建仓窗 → 仓库就绪之后才出「该工作区尚未初始化」黄条 → 点黄条弹布局小卡 → 选完注入初始化全文。此前这条路有三处不对：仓库还没建好就提前跳出黄条，建仓窗挂在右侧面板上（面板没开就点了什么都不发生），点完后端还要干等一截。
- **顺序只有一份真源**：那条链的每一步收进 `src/shared/tracker/guide-steps.js` 一份清单，宿主按它排序、状态栏横幅按它出、检查页按它排（#668）。横幅因此不再跳步，黄条一定落在仓库那一步之后（#663、#664、#665）。
- **点完之后不用干等**：点「确认并继续」时立刻重取一次链（真机实测：点确认到横幅出现，11.4 秒 → 1.7 秒），并且把晚到的旧结果丢弃，不再让旧数据盖回新快照（#669 的这两处，提交 `6d074ed` 与 `7208564`）。
- **建仓窗搬到状态舱那一层**：「创建并发布」那颗弹窗的生命原先绑在右侧面板的检查页上 —— 面板没开、或面板停在别的页，点它什么都不会发生；现在它挂在输入区那一条状态舱上，盖在整个界面之上，与面板开不开无关（#669、#671）。
- **收起之后看得见回来的路**：横幅最右那颗叉会把整个功能区收起来。收起之后留在原地的那颗按钮写着面板全名，点它横幅与胶囊一起回来、收起标记被清空（这是 #422 要的行为）；那颗叉与左边的主按钮之间拉开 12 像素，免得误点（#669，提交 `51372bc`、`0d5937f`）。
- **初始化那张小卡不再问一遍后端**：走到 /setup 这一步时，卡上原来又摆着后端三选一（明明在选后端那个窗里刚选过），而且卡上那颗「确认并继续」会顺手把这个工作区的后端重新绑一遍 —— 一张看起来只是「答个文档布局」的卡能把整个工作区的后端换掉。现在卡上只剩「域文档布局」这一问，另加一行只读说明写清这次用哪个后端、去哪儿换（#669 第 4 件，提交 `43f57d3`、`9fbfb46`）。
- **切换后端点确认没反应，修好了**：面板里选另一个后端、点「确认切换」，如果这之前刚好飞着一次刷新，绑定成功之后那次重取会被当成同一次请求复用掉 —— 一个新请求都不发，界面拿回的是切换前那份数据，于是「点了确认没有任何反应」。现在换过后端就一定真发一次重取，并且晚到的旧数据不再写进会话状态；这条电话（`wf.bind`）也补上了日志（#669 第 5 件，提交 `df79fcc`）。
- **那张切换弹窗在窄面板下被挤坏，也修好了**：面板一窄，标题行会把三颗按钮压成竖排（「清除后端选择」六个字竖着排六行）、确认按钮被推出卡片右边、✕ 被挤出去。现在按钮整组换行到第二行、每颗按钮都只有一行字、标题超出留省略号；标题本身也按维护者要求从「切换 Tracker 后端」缩短成「切换后端」（#669 第 5 件，提交 `df79fcc`）。
- **面板这几处**：面板头部去掉品牌图标与文字（#667）；面板只剩 DSH 原生右侧边栏一条落点，页内浮窗那条老路退役（#646）；面板第一帧不再是透亮的空块（#670）；「当前目录属于工作区 X」那枚标志修好并改红（#666）。
- **对应提交**：`2a37464`（#667 面板头）、`6d074ed`（绑完立刻重取链 + 弹窗搬到状态舱）、`311636e`（面板只留原生右栏 + 第一帧底色）、`03573dd`（#666 那枚标志）、`51372bc` 与 `0d5937f`（#669 收起态看得见回来的路）、`7208564`（#669 晚到的旧结果不许盖回）、`43f57d3` 与 `9fbfb46`（#669 第 4 件：初始化那张小卡只问布局）、`df79fcc`（#669 第 5 件：切换后端点确认没反应 + 窄面板下那张弹窗），以及本版发布提交。
- **验证**：`node tests/verify-release-contract.js --version v1.7.23` 全绿；`npm run verify` 整条链全绿（126 道门禁；本版新增四道：收起态「看得见、回得来」的真浏览器几何门、点确认后立刻重取链的行为门、弹窗挂到状态舱的全屏遮罩门、切换后端那条路的四组门 24 项）；`npm run test:smoke` 全绿；`npm pack --dry-run` 只含白名单文件。真机逐条验收由维护者在本版发布前按清单走（场景、步骤与通过标准见 #669 的评论）。
- **影响**：装到本版后，新工作区第一次打开会按顺序出提示，不会提前跳黄条；建仓弹窗不再依赖右侧面板；/setup 那一步只问布局；切换后端点确认会当场看到结果；点过收起也随时能展开回来。面板其余行为与 `1.7.22` 一致。

## 2026-09-17 · v1.7.22 发布：标签配色可编辑——面板里给标签挑颜色，GitHub 与本地 Markdown 两个后端都能改

- **目的**：在这之前，标签用什么颜色是写死在代码和初始化脚本里的，用户看着不顺眼也改不了。本版在面板头部放一个调色盘入口，点开就是一张标签全表，每行一个取色控件，可以逐行改、也可以整批改完一次保存。改的是哪条跑道上的标签，就落到哪条跑道：GitHub 后端调 GitHub 的改标签接口，本地 Markdown 后端写进工作区里的一份配色文件。
- **这一版是标签配色第一次真正到达用户手里。** 已经发到官方源的 `1.7.21` 里**完全没有**这部分代码：把那个 tarball 下载下来逐字比对，它一次都没有出现 `wf.listLabels`、`wf.setLabelColors`、`label-colors` 这些名字，产物比本版小约 89 KB。原因是这一整套改动都在 `1.7.21` 打标签之后的第二天（2026-09-13）才落到主线，此后一直待在主线上，没有随任何一版发出去。
- **配色能力本体**：配色的三个核心模块（颜色写法、后端接口形状、写给 agent 的配色提示词）用 TypeScript 写、但随插件内置发布，不额外发 npm 包（裁决见 `docs/adr/20260913-builtin-ts-shape.md`）。颜色统一写成不带井号的六位小写十六进制，空串表示「这条标签还没配色」。
- **两个后端各自的路**：GitHub 后端走 `gh label edit` 批量改色，八档失败原因分开报，不混成一句「失败了」；本地 Markdown 后端把配色写进工作区里的 `docs/agents/label-colors.json`，取代了旧的那张写在文档表格里的调色盘。旧的 `docs/adr/20260828-markdown-map-and-label-palette.md` 里关于「调色盘与 setup 同位、标签表扩 Color 列」的那部分已作废并标注。
- **界面上看得见的改动**：面板头部那颗入口图标整只上色，让人一眼认出是调色盘（#637）；改色弹窗在保存成功后，面板当场就按新颜色显示、弹窗自己关掉（#635）；弹窗里有一个「复制推荐配色方案」按钮，GitHub 版与本地 Markdown 版各一套文案，复制出来的提示词可以直接交给 AI（#622）。
- **这一批里修掉的缺陷**：本地 Markdown 票面的标签名带着一对反引号、颜色也回落到灰色（#634）；新会话顶栏显示空工作区，原因是查找工作区编号时快照缺了一个形状，补齐后新会话重新选中当前工作区（#639）；地图详情页的普通票印出字面量 `type.issue`，改成显示「普通票」（#626）。
- **顺带恢复的门禁**：新会话查找工作区编号的那段代码原本让主分支上的文件粒度门禁一直是红的（`api-new-session.js` 430 行、上限 350 行），本次把整块搬进新模块，门禁恢复全绿（#636）。
- **文案整改**：标签配色所有用户可见的文字改成书面、专业的表述，并同步改准了被钉住的断言（#633）——此前实现过程里的说法漏到了界面文案里。
- **另外两件用户看得见的小事**：状态栏「可接」悬停多出一个紫色地图图标，点它就能提新需求，和面板里「加需求」走同一条路（#638）；注入给 agent 的两块提示词（正文格式块、子议题关联块）本版没有改动，与 `1.7.19` 以来发出去的写法逐字一致。
- **对应提交**：`c58d235..fd8d1cf` 区间的 29 笔提交（标签配色 #618–#637、状态栏加需求入口 #638、新会话工作区 #636/#639、普通票显示 #626）与本版发布提交。
- **验证**：发布契约校验 `node tests/verify-release-contract.js --version v1.7.22` 61 项全绿；`npm run verify` 整条链全绿；`npm run test:smoke` 全绿；`npm pack --dry-run` 只含白名单文件。构建与全部校验都在隔离目录里跑（`--no-sync`，不碰已装形态）。真机验收（两个后端、失败路径）由票 #623 完成，记录见票内评论。本版发布前后，仍在 DSH 官方源当前的 `latest` 内核 `0.1.5-rc.1` 与 DSH Desktop 2.0.10 自带的内核 `0.1.5-rc.2` 上复核面板行为。
- **影响**：装到本版后，面板头部多出一个调色盘入口，点开就能查看与修改标签颜色；改完保存，面板立刻按新颜色显示。GitHub 后端需要账号有改标签的权限，权限不够时会写明是哪一类原因。本地 Markdown 后端的配色落在工作区里，跟着仓库走。面板其余行为与 `1.7.21` 一致。
