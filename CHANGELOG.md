...
- **目的**：修三处真机与日志里看到的具体毛病，并把两处取数口径收成一份。降级横幅此前只要走了备用通道就说配额耗尽；手动点刷新一定另起一整趟，大工作区上越点越慢；切进工作区时处理链只写盘不读回，重启后内存丢失；同一会话的目录在两处各取一遍，答案可能不一致。
- **改了什么（按能看到的行为）**：
  - **降级横幅**：只有宿主明确说是配额耗尽才提配额二字，其它失败与未知原因走中性说法（此前账号还剩 4983 点也会误报配额耗尽）；快照里新增原因三档（配额耗尽 / 其它失败 / 未知），界面只读宿主写下的结论，不自己猜。
  - **手动刷新**：自动刷新的快照路与手动点的刷新路共用一张在途表，强制只搭强制的车、非强制搭任何车；大工作区上连点不再越点越慢，也不再多花额度。
  - **处理链**：切进工作区时把盘里的处理链读回一格，重启后内存不再丢失。
  - **会话目录**：两处取法收成共用函数，同一会话两处答案一致；无证据的猜测槽位删除，缺席时按无号、无服务、找不到、无目录四种如实报失败，不再静默给默认值。
  - **日志 731**：常驻放行修好，开关关着也能从日志回答慢问题；计数同步为常驻 38 条、按需 43 条、自监控 5 条、总数 86 条。
  - **不认识电话留痕**：新界面调旧宿主导致不认识电话时，宿主侧记一行电话名与服务版本，升级窗口的失败能定位到版本，不再只能靠客户端反推。
- **如实说明**：自 v1.7.29 发布后主线上有 23 笔提交，本版实际发出全部代码；其中子目录首帧、两条回包共用信封、胶囊与横幅、会话读数重做与下线等 12 笔，已在 v1.7.29 变更历史补写中提及但当时并未随包发出，本次随 v1.7.30 一起发出，不再重复描述；本节只详述本次新增的 11 笔（`512be48`·`fca9b96`·`5fdc0de`·`b73237f`·`594f3e4`·`d939896`·`b365446`·`5e02638`·`9c24c8f`·`e7d6e58`·`161c76a`）。
- **对应提交**：见上一条如实说明中的 11 笔，以及本版发布提交。
- **验证**：`node tests/verify-release-contract.js --version v1.7.30` 61 通过 / 0 失败；`npm run test:smoke` 全绿；本版新增与改动的门禁逐条单跑全绿（`verify-733-unknown-endpoint`、`verify-731-resident` 22 项、`verify-729-force-dedup` 39 条、`verify-735-chain-restore` 9 项、`verify-730-cwd-consistent` 27 项、`verify-rest-fallback-reason`、`verify-log-count` 98 项总数 86、`verify-chain-view-ui` 22 条、`verify-reply-envelope-parity` 两条各 23 项、`verify-subdir-first-frame` 52 项）；`node scripts/build.mjs` 三行 profile 同步 hash 校验通过。全链用不短路的 `node scripts/verify-all.mjs` 跑完 **181 道、178 通过 / 3 失败**，红的正好是开工前就存在的三条存量红（`verify-3-workspace-switch`、`verify-no-same-layer-import`、`verify-multi-effort-panel`），本版没有把它们算作通过，也没有新增红灯。**关键修复逐条做过回滚反证**（`verify-729` 整树回滚、`verify-730` 回滚变红恢复变绿、`verify-731` 回滚变红恢复变绿、`verify-735` 负控非恒绿、`verify-rest-fallback-reason` 非配额不再误报，均在各提交正文留痕）。
- **影响**：装到本版后，降级横幅不再误报配额耗尽；大工作区上手动点刷新不再越点越慢；重启后处理链还在；会话目录缺席时界面如实说失败原因；日志开关关着也能查慢问题；升级窗口调到旧宿主时的失败能在宿主日志里定位到版本。面板其余行为与 `1.7.29` 一致。

## 2026-09-24 · v1.7.29 发布：刷新额度收进机制、票务工具不再绑定某个后端；提示词卸下两套旧契约、面板与胶囊的一批真机毛病逐个修好

### 提示词卸下两套旧规矩、面板与胶囊的一批真机毛病修好、会话读数先下线（能力保留）

- **目的**：这一版做两件事。一是把提示词里两段「只能手敲跟踪器命令」时代的契约删掉（正文格式、进度规矩），把它们交回给工具与 AI 自己；二是把维护者在真机上逐个发现的界面毛病修掉，并把「能力」与「展示」分开——能力留着，展示该下的先下。
- **改了什么（按能看到的行为）**：
  - **提示词**：删掉「## 正文格式」整节（十一个动作模板末尾那段写法规矩、三个后端的正文格式声明、注册表兜底版，以及客户端按后端解析它的整条渲染链 `bodyFormatText` / `BODY_FORMAT` 与两处追加点）；删掉进度契约（不再要求 `## 进度：N%` 固定写法、不再有 0/1-94/95/100 的语义阶梯、取消「未确认不得 close」）；带 needs-triage 的诊断闸门改看「正文 / 评论 / 标签 / 已有的实施记录」判断阶段；顺带删掉两处「先把正文取下来存成文件、改好再写回」的中间文件要求。
  - **面板头部**：三颗小按钮（归属标志 / 切换后端 / 标签配色）外框统一成 16×16、圆角 4 —— 此前「切换后端」与「标签配色」漏写 `box-sizing`，实际画成 18×18，比归属标志大一圈；另外把那枚归属标志比邻居高 0.7 像素的行盒问题一并修掉。加载中那一栏不再整块空白，改成一条定宽灰条占位（不带折叠机认的标记，芯片到了不跳行）。
  - **胶囊状态栏**：品牌那串字改成「**起手折叠、量到有宽就显示**」（首帧是折叠态，只当一次起始态；宽度不够才保持折叠，且它第一个让位、字一个一个减少，直到只剩图标与数字——**计数器数字与图标任何时候都不参与折叠**）；各项**均匀分布**在整条里（此前是一个元素吃掉了全部余量，把右边一串顶成一簇、中间留 150 像素大洞）；图标与它自己的文字合成一个整体（此前那条随视口放大的间隔把它俩撑到 28 像素）。
  - **降级横幅**（GraphQL 配额耗尽已切 REST）：把「历史事实」与「当下通道」拆开——带发生时刻，一小时以内照旧说「已切换 REST 通道」，超过一小时或拿不到时刻就改口「上次取数走的 REST（N 分钟前）」，并给一颗「本次会话不再提示」的关闭钮；顺带去掉重复的第二个警告三角。
  - **子目录会话首帧「没有后端」**（真机报的那个）：进工作区时补问一次现成的便宜电话 `wf.selection`（不再让「这个工作区用哪个后端」搭最重的那趟快照车）、迟到的成功回包也能落地、子目录那份回包的过期判据改成按「请求发出时那条目录」比。
  - **「读不到处理记录」常驻**：两条快照回包路径（`wf.snapshot` 与 `wf.refresh`）改成共用同一份信封——此前强制刷新那条根本没挂这份读数，于是界面只要走过一次强制刷新就一直显示「读不到」；同时把「还没有记录」与「真读不到」分开（前者不占版面，后者才可见且带原因代号）。
  - **「每个会话在处理哪些票」展示面暂时下线**：按维护者决定不再在面板里显示，但**能力全留**（宿主侧的链、写事件订阅、快照字段、两条回包路径字段对等；客户端的判据、组件、词条、导出全在），下线处留了指针写明去向与原因。
- **为什么这么做**：提示词那两段是「还没有工具」时的替代品，现在工具在手，规矩只占提示词；界面那一批全是真机上量出来的具体毛病（哪一处多 2 像素、哪一处吃掉全部余量、哪一句话在说历史却在冒充现在），修法都按「先说清机制、再改、再配门禁」办；展示面下线是因为它的呈现不被接受，而能力本身还有别的用处（写事件白名单与配额归因）。
- **对应提交**：`7cc216d`（提示词两套契约）· `207847a`（面板与胶囊六件 + 降级横幅）· `b9583cb`（胶囊折空回归）· `624815e`（子目录首帧「没有后端」）· `59b7056`（两条回包路径共用信封）· `ad7c671`（品牌字起始态）· `7557354`（三角去重 + 胶囊均分）· `bea58bd`（会话读数重做）· `29dd75d`（展示面下线、能力保留）· `05dc23f`（变更历史与功能自检清单同步），以及本版发布前的收尾提交（补回版面行话的那两把门禁尺子）。
- **验证**：全链用**不短路**的 `node scripts/verify-all.mjs` 跑完 **177 道、174 通过 / 3 失败**，红的正好是开工前就存在的三条存量红（`verify-3-workspace-switch`、`verify-no-same-layer-import`、`verify-multi-effort-panel`），本版没有把它们算作通过。本版新增/强化的门禁：`verify-cap-fold`（54 条，纯函数阶梯）、`verify-cap-fold-browser`（44 条，真 Chromium）、`verify-capsule-layout`（17 条，四档宽度的间隔均匀与图标↔文字间隙）、`verify-chain-view-ui`（20 → 22 条：断言面板里已没有那一块、能力本体仍画得出来，并补回「版面不许出现 8 位十六进制散列与内部行话」以及它的反向自检这两把尺子）、`verify-reply-envelope-parity`（两条回包路径字段逐项相等）、`verify-subdir-first-frame`（52 项，含四条反证）、`verify-rest-fallback-banner`、`verify-667` 新增三颗按钮同宽高同中心（91 条）。**关键修复逐条做过回滚反证**（把修复退回上一版 → 门禁变红 → 恢复 → 变绿）：胶囊折空、两条回包字段对等、会话读数展示面下线这三条我都亲自复跑过；另外有一次我给折叠判据做的人工改坏没有打中那条守卫，当时如实记成「这条反证无效」，没有把它算作已证明。补回的那两把尺子做了两次真反证：把行话判据改坏成永远不命中 → 反向自检那条当场红（21 通过 / 1 失败、退出码 1）；把 8 位散列塞进真版面 → 「版面干净」那条当场红（同样 21 / 1、退出码 1）；两种改动都还原后回到 22 通过 / 0 失败。我自己另做了一次独立反证（只塞 8 位散列、一个字行话都不塞）也打中了那一条（实测命中散列 1 行、行话 0 行）。`node tests/verify-release-contract.js --version v1.7.29` 全绿；`node scripts/build.mjs` 三行 profile 同步 hash 校验通过。
- **影响**：装到本版后，注入会话的动作提示词明显变短；面板头部那三颗按钮齐平、载入瞬间不再整块空白；胶囊里的字随宽度逐字让位、各项均匀分布、品牌字在宽条上看得见；降级横幅不再常驻、也更诚实；子目录会话第一次进来就能认出「这个工作区用哪个后端」；那句「读不到处理记录」不会再冒出来。**如实说明三件**：① `deck_*` 七个工具仍只在插件进程内可用（还没挂到 DSH 的模型工具面上，跟踪票 #725），所以「票的读写走工具」对新会话里的 AI 还差最后一步接线；② 「每个会话在处理哪些票」这块界面按维护者决定暂时下线（能力保留，将来按他指的票号在别处挂回）；③ 这一轮排查中发现的另外五处缺口（强制刷新另起一趟、`wf.cwd` 偶发答不出、日志不记耗时、会话记录的下落没查清、视野心跳一天失败 303 次）已各自开票挂在地图 #704 之下，本版没有修它们。
### 刷新额度与刷新的节拍收进机制、票与地图的工具不再绑定某个后端、界面按真实读数说话
...
- **改了什么（按能看到的行为）**：
  ...
  - 界面上新加一条「每个会话在处理哪些票」，数据只读宿主写下的真实读数：**读不到就如实说读不到**；宿主说取到了而确实没有，整块不画。（这一块在本版晚些时候已按维护者口径从面板下线、能力保留，见上一节。）
