...
- **对应提交**：`7cc216d`（提示词两套契约）· `207847a`（面板与胶囊六件 + 降级横幅）· `b9583cb`（胶囊折空回归）· `624815e`（子目录首帧「没有后端」）· `59b7056`（两条回包路径共用信封）· `ad7c671`（品牌字起始态）· `7557354`（三角去重 + 胶囊均分）· `bea58bd`（会话读数重做）· `29dd75d`（展示面下线、能力保留），以及本版发布提交。
- **验证**：全链用**不短路**的 `node scripts/verify-all.mjs` 跑完 **177 道、174 通过 / 3 失败**，红的正好是开工前就存在的三条存量红（`verify-3-workspace-switch`、`verify-no-same-layer-import`、`verify-multi-effort-panel`），本版没有把它们算作通过。本版新增/强化的门禁：`verify-cap-fold`（54 条，纯函数阶梯）、`verify-cap-fold-browser`（44 条，真 Chromium）、`verify-capsule-layout`（17 条，四档宽度的间隔均匀与图标↔文字间隙）、`verify-chain-view-ui`（20 条，断言面板里已没有那一块、能力本体仍画得出来）、`verify-reply-envelope-parity`（两条回包路径字段逐项相等）、`verify-subdir-first-frame`（52 项，含四条反证）、`verify-rest-fallback-banner`、`verify-667` 新增三颗按钮同宽高同中心（91 条）。**关键修复逐条做过回滚反证**（把修复退回上一版 → 门禁变红 → 恢复 → 变绿）：胶囊折空、两条回包字段对等、会话读数展示面下线这三条我都亲自复跑过；另外有一次我给折叠判据做的人工改坏没有打中那条守卫，当时如实记成「这条反证无效」，没有把它算作已证明。`node tests/verify-release-contract.js --version v1.7.29` 全绿；`node scripts/build.mjs` 三行 profile 同步 hash 校验通过。
