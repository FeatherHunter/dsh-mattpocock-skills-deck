...
- **验证**：`node tests/verify-prompts.js` 与 `node tests/verify-progress.js` 全绿，`npm run verify` 链全绿；反面实证：把旧的两步走文本塞回 GitHub 后端会立刻变红。
...
- **提示词第 ① 步修复（#600）**：注入给 agent 的两处正文写回提示词（正文格式块与子议题关联块，中英各一份）里，第 ① 步那条命令补上必填的 `--profile`——旧写法 `dsh plugin exec …` 会让 agent 卡在第 ① 步（`dsh` 直接报 `required option '--profile <name>' not specified`）；现在改成 `dsh plugin --profile <配置名> exec …` 并写明这个值填「启动这个界面时用的那个 profile 名，Web 界面通常就是 web」，照抄即可跑通。门禁同步钉住第 ① 步的精确串（中文 `<配置名>`、英文 `<profile>` 各一份）与「不带参数的旧写法不许回来」。
